defmodule SecretHub.Web.PolicyEditorLive do
  @moduledoc """
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
                <%= for {tab_id, tab_name} <- [
                  ...
                  <button
                    ...
                    class={"whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm #{if @active_tab == tab_id, do: "border-primary text-primary", else: "border-transparent text-on-surface-variant hover:text-on-surface hover:border-outline-variant"}"}
                  >
                    {tab_name}
                  </button>
                <% end %>
        ...
        <div class="lg:col-span-1">
          <div class="bg-surface-container shadow rounded-lg p-6 sticky top-6">
            <h3 class="text-lg font-medium text-on-surface mb-4">Policy Document Preview</h3>
            <div class="bg-surface-container-low rounded-md p-3 overflow-x-auto">
              <pre class="text-xs text-on-surface">{Jason.encode!(build_policy_document(@form_data), pretty: true)}</pre>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="space-y-6">
      <div>
        <label class="block text-sm font-medium text-on-surface">
          Policy Name <span class="text-error">*</span>
        </label>
        <input
          type="text"
      ...
      <div class="flex items-start">
        ...
        <div class="ml-3">
          <label for="deny_policy" class="font-medium text-on-surface">Deny Policy</label>
    ...
    <div class="space-y-6">
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">
          Allowed Secret Patterns <span class="text-error">*</span>
        </label>
        ...
        <div class="mb-4">
          ...
          <ul class="text-xs text-on-surface-variant space-y-1">
            <li>
              <code class="px-1 py-0.5 bg-surface-container rounded">prod.db.*</code>
              - Matches all database secrets in production
            </li>
            <li>
              <code class="px-1 py-0.5 bg-surface-container rounded">*.password</code>
              - Matches all password secrets
            </li>
            <li>
              <code class="px-1 py-0.5 bg-surface-container rounded">dev.*</code>
        ...
        <div class="flex flex-wrap gap-2">
          <%= if Enum.empty?(@form_data["allowed_secrets"]) do %>
            <p class="text-sm text-on-surface-variant italic">No patterns added yet</p>
          <% else %>
            <%= for pattern <- @form_data["allowed_secrets"] do %>
              <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary/10 text-primary">
                ...
                <button
                  ...
                  class="ml-2 text-primary hover:text-primary font-bold"
                >
                  &times;
                </button>
              </span>
  ...
  defp render_operations_tab(assigns) do
    ...
    <div class="space-y-6">
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">
          Allowed Operations <span class="text-error">*</span>
        </label>
        <p class="text-sm text-on-surface-variant mb-4">
        ...
        <div class="space-y-3">
          <%= for {op, description} <- [
            ...
            <label class="flex items-start">
              ...
              <div class="ml-3">
                <span class="font-medium text-on-surface">{String.capitalize(op)}</span>
  ...
  defp render_conditions_tab(assigns) do
    ...
    <div class="space-y-6">
      ...
      <!-- Time of Day -->
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">Time of Day</label>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs text-on-surface-variant mb-1">Start Time</label>
            <input
              type="time"
              id="start-time"
          ...
          <div>
            <label class="block text-xs text-on-surface-variant mb-1">End Time</label>
            <input
              type="time"
              id="end-time"
        ...
        <p class="mt-1 text-xs text-on-surface-variant">Restrict access to specific hours (UTC)</p>
      </div>
      <!-- Days of Week -->
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">Days of Week</label>
          ...
          <%= for day <- ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"] do %>
            <label class="flex items-center">
              <input
                ...
                class="h-4 w-4 text-primary focus:ring-primary border-outline-variant rounded"
              />
              <span class="ml-2 text-sm text-on-surface">{String.capitalize(day)}</span>
            </label>
          <% end %>
        </div>
      </div>
      <!-- Date Range -->
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">Date Range</label>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs text-on-surface-variant mb-1">Start Date</label>
            <input
              type="date"
              id="start-date"
              class="block w-full border border-outline-variant rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
          ...
          <div>
            <label class="block text-xs text-on-surface-variant mb-1">End Date</label>
            <input
              type="date"
              id="end-date"
              phx-blur="update_condition"
      ...
      <!-- IP Ranges -->
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">IP Ranges (CIDR)</label>
        <textarea
          id="ip-ranges"
          phx-blur="update_condition"
          ...
          rows="3"
          class="block w-full border border-outline-variant rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm font-mono text-xs"
          placeholder="10.0.0.0/8\n192.168.1.0/24"
        >{format_ip_ranges(@form_data["conditions"])}</textarea>
        <p class="mt-1 text-xs text-on-surface-variant">One CIDR block per line</p>
      </div>
      <!-- Max TTL -->
      <div>
        <label class="block text-sm font-medium text-on-surface mb-2">Maximum TTL (seconds)</label>
        <input
          type="number"
          phx-blur="update_condition"
            ...
            <%= for entity_id <- @form_data["entity_bindings"] do %>
              <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-success/10 text-success">
                ...
                <button
                  ...
                  class="ml-2 text-success hover:text-success font-bold"
                >
                  &times;
                </button>
              </span>
